"""Unified TUI dashboard - all output in one terminal screen using rich."""
...
class Dashboard:
    """Unified TUI dashboard combining all network diagnostics."""
    ...
    def _make_header(self) -> Panel:
        ...
        title = Text()
        title.append("  WiFi Network Diagnostics  ", style="bold white on blue")
    ...
    def _make_iperf_panel(self) -> Panel:
        if not self.iperf_tester:
            return Panel("iperf3: not configured", title="Bandwidth", border_style="dim")
        ...
        return Panel(table, title="[bold]Bandwidth (iperf3)[/]", border_style="magenta")
